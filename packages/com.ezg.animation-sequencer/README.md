<!--
EZG note: this package is `com.ezg.animation-sequencer`, a vendored copy of
brunomikoski/Animation-Sequencer republished under the com.ezg scoped registry
so EZG projects can consume it via the existing `com.ezg` scope.

- Source folder in the game repo: Assets/_Project/3rdParty/Animation-Sequencer
- Assemblies (unchanged): BrunoMikoski.AnimationSequencer (Runtime),
  BrunoMikoski.AnimationSequencer.Editor (Editor)
- Peer requirements (NOT bundled — the consuming project must already provide
  them): DOTween, UniTask, TextMeshPro, and I2 Text Animation (referenced by
  the "I2TextAnimation" assembly; the PlayTextAnimationStep / TMP steps need it).
- Original author/license/links below are preserved as-is.
-->
