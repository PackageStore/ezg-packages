...
namespace TigerForge
{
    ...
    public class EventManager
    {
        ...
        /// </summary>
        public static void StartListening(string eventName, UnityAction callBack, string callBackID = "", bool isRemove = true)
        {
        ...
        public static Vector2 GetVector2(string eventName)
        {
            try
            {
                if (storage.ContainsKey(eventName)) return (Vector2)storage[eventName]; else return default(Vector2);
            }
            catch (System.Exception)
            {
        ...
        public static T GetData<T>(string eventName)
        {
            try
            {
                if (storage.ContainsKey(eventName)) return (T)storage[eventName]; else return default(T);
            }
            catch (System.Exception)
            {
                return default(T);
        ...
        /// <summary>
        /// Set data và emit luôn
        /// </summary>
        public static void EmitEventDataGroup(string eventName, params object[] data)
        {
            if (storage3.ContainsKey(eventName)) { Debug.LogWarning(eventName + " Event name is already in use with DataGroup."); return; }
            if (storage2.ContainsKey(eventName)) storage2[eventName] = data; else storage2.Add(eventName, data);
            EmitEvent(eventName);
