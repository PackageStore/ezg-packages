        ...
        private static void FindAndExecuteOnScriptHotReload(Type type)
        {
            ...
            if (onScriptHotReloadFnForType != null)
            {
                UnityMainThreadDispatcher.Instance.Enqueue(() =>
                {
                    ...
                    foreach (var instanceOfType in GameObject.FindObjectsByType(type, FindObjectsInactive.Include, FindObjectsSortMode.None)) //TODO: perf - could find them in different way?
                    {
