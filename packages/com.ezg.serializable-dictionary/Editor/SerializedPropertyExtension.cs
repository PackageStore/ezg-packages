...
{
    /// <summary>
    ///     Extension and utility methods for SerializedProperty.
    /// </summary>
    public static class SerializedPropertyExtension
    {
        ...
        /// <summary>
        ///     Generates a unique hash code for the given SerializedProperty based on its path and serialized object.
        /// </summary>
        /// <param name="p">The SerializedProperty.</param>
        /// <returns>A unique integer hash code.</returns>
        ...
        /// <summary>
        ///     Compares the basic properties of two SerializedProperty instances for equality.
        /// </summary>
        /// <param name="left">The first SerializedProperty.</param>
        /// <param name="right">The second SerializedProperty.</param>
        /// <returns>True if their basic values are equal, otherwise false.</returns>
        public static bool EqualBasics(SerializedProperty left, SerializedProperty right)
        {
            ...
            if (left.propertyType == SerializedPropertyType.String) return left.stringValue == right.stringValue;

            if (left.propertyType == SerializedPropertyType.ObjectReference)
                return left.objectReferenceValue == right.objectReferenceValue;

            if (left.propertyType == SerializedPropertyType.Enum) return left.enumValueIndex == right.enumValueIndex;

            if (left.propertyType == SerializedPropertyType.Boolean) return left.boolValue == right.boolValue;
            ...
            if (left.propertyType == SerializedPropertyType.Color) return left.colorValue == right.colorValue;

            if (left.propertyType == SerializedPropertyType.LayerMask) return left.intValue == right.intValue;

            if (left.propertyType == SerializedPropertyType.Vector2) return left.vector2Value == right.vector2Value;

            if (left.propertyType == SerializedPropertyType.Vector3) return left.vector3Value == right.vector3Value;

            if (left.propertyType == SerializedPropertyType.Vector4) return left.vector4Value == right.vector4Value;

            if (left.propertyType == SerializedPropertyType.Rect) return left.rectValue == right.rectValue;

            if (left.propertyType == SerializedPropertyType.ArraySize) return left.arraySize == right.arraySize;

            if (left.propertyType == SerializedPropertyType.Character) return left.intValue == right.intValue;

            if (left.propertyType == SerializedPropertyType.AnimationCurve) return false;

            if (left.propertyType == SerializedPropertyType.Bounds) return left.boundsValue == right.boundsValue;

            if (left.propertyType == SerializedPropertyType.Gradient) return false;
        ...
        /// <summary>
        ///     Copies basic property values from the source SerializedProperty to the target SerializedProperty.
        /// </summary>
        /// <param name="source">The source SerializedProperty.</param>
        /// <param name="target">The target SerializedProperty.</param>
