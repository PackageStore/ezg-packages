...
- `build_unity_template.sh` — **bootstrap mỏng** (~150 dòng). Đây là file duy nhất end user giữ trên máy. Nó gần như không bao giờ thay đổi. Mỗi lần chạy, nó tải logic mới nhất từ server, verify SHA-256, rồi bàn giao quyền điều khiển.
...
```text
EzgFeatureHub/
├─ build_unity_template.sh        # bootstrap (file duy nhất end user giữ)
├─ unity-template.json
...
- `build_unity_template.sh`: bootstrap mỏng — tải và chạy logic build mới nhất từ server (xem mục [Kiến trúc: bootstrap + logic](#kiến-trúc-bootstrap--logic)).
