...
api_post_json() {
  local url="$1" body="$2" out="$3" token="${4:-}"

  if command -v curl >/dev/null 2>&1; then
    if [ -n "$token" ]; then
      curl -sS -o "$out" -w '%{http_code}' -X POST -H 'Content-Type: application/json' \
        -H "Authorization: Bearer $token" --data "$body" "$url" 2>/dev/null || printf '000'
    else
      curl -sS -o "$out" -w '%{http_code}' -X POST -H 'Content-Type: application/json' \
  ...
  if [ "$OS_NAME" = "windows" ] && command -v powershell.exe >/dev/null 2>&1; then
    EZG_URL="$url" EZG_BODY="$body" EZG_OUT="$(to_unity_arg_path "$out")" EZG_BEARER="$token" \
      powershell.exe -NoProfile -ExecutionPolicy Bypass -Command '
        try {
          $headers = @{}
          if ($env:EZG_BEARER) { $headers["Authorization"] = "Bearer " + $env:EZG_BEARER }
          $response = Invoke-WebRequest -Uri $env:EZG_URL -Method POST -ContentType "application/json" `
            -Body $env:EZG_BODY -Headers $headers -UseBasicParsing
...
# Trade a still-valid token for one with a fresh window. Best-effort: a failure here just means the
# current token keeps its remaining lifetime, which is enough to finish this build.
ezg_refresh_token() {
  local tmp status body new_token

  [ -n "$EZG_TOKEN" ] || return 1
  tmp="$(mktemp 2>/dev/null || printf '%s' "${TMPDIR:-/tmp}/ezg-refresh.$$")"
  status="$(api_post_json "$GATEWAY_URL/auth/refresh" '{}' "$tmp" "$EZG_TOKEN")"
  body="$(cat "$tmp" 2>/dev/null || printf '{}')"
  rm -f "$tmp"

  [ "$status" = "200" ] || return 1
  new_token="$(json_field "$body" access_token)"
...
# Write a tiny standalone session-refresher into ~/.ezg/, which the per-project launcher calls right
# before it starts Unity.
#
# Why this exists at all: Unity's Package Manager reads the registry token from ~/.upmconfig.toml when
# the Editor process starts and cannot sign in again by itself. So the token has to be fresh BEFORE
# Unity launches -- refreshing from inside the Editor is already too late for that first resolve.
# One shared copy per machine rather than one per project, so every project picks up fixes.
write_session_helper() {
  mkdir -p "$EZG_CRED_DIR" 2>/dev/null || return 0

  # Both helpers are written from QUOTED heredocs so the shell leaves their contents alone -- the
  # PowerShell one is full of $vars and backticks that bash would otherwise eat -- and the single
  # gateway placeholder is substituted afterwards.
  cat >"$EZG_CRED_DIR/refresh-session.sh.tmp" <<'HELPER_SH'
#!/usr/bin/env bash
# Auto-generated by build_unity_template. Refreshes the EZG session and updates ~/.upmconfig.toml.
# Best-effort by design: never block opening Unity just because the network is down.
set -u
GATEWAY="${EZG_GATEWAY_URL:-__EZG_GATEWAY__}"
CRED="${EZG_CRED_FILE:-$HOME/.ezg/credentials.json}"
UPMCONFIG="${UPM_USER_CONFIG_PATH:-$HOME/.upmconfig.toml}"

command -v curl >/dev/null 2>&1 || exit 0
command -v python3 >/dev/null 2>&1 || exit 0
[ -f "$CRED" ] || exit 0

OLD_TOKEN="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get("access_token",""))' "$CRED" 2>/dev/null)"
[ -n "$OLD_TOKEN" ] || exit 0

BODY="$(curl -fsS -X POST -H "Authorization: Bearer $OLD_TOKEN" "$GATEWAY/auth/refresh" 2>/dev/null)" || exit 0
NEW_TOKEN="$(printf '%s' "$BODY" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("access_token",""))' 2>/dev/null)"
[ -n "$NEW_TOKEN" ] || exit 0

printf '%s\n' "$BODY" >"$CRED"
chmod 600 "$CRED" 2>/dev/null || true

python3 - "$UPMCONFIG" "$GATEWAY" "$NEW_TOKEN" <<'PYUPM'
import io, os, re, sys
path, registry, token = sys.argv[1], sys.argv[2].rstrip("/"), sys.argv[3]
existing = ""
if os.path.exists(path):
...
HELPER_SH
  ...
  cat >"$EZG_CRED_DIR/refresh-session.ps1.tmp" <<'HELPER_PS1'
# Auto-generated by build_unity_template. Windows counterpart of refresh-session.sh.
$ErrorActionPreference = "SilentlyContinue"
$gateway = "__EZG_GATEWAY__".TrimEnd("/")
$cred = Join-Path $env:USERPROFILE ".ezg\credentials.json"
$upmconfig = Join-Path $env:USERPROFILE ".upmconfig.toml"

if (-not (Test-Path -LiteralPath $cred)) { exit 0 }
$old = (Get-Content -LiteralPath $cred -Raw | ConvertFrom-Json).access_token
if (-not $old) { exit 0 }

try {
  $body = Invoke-RestMethod -Uri "$gateway/auth/refresh" -Method POST -Headers @{ Authorization = "Bearer $old" }
} catch { exit 0 }
if (-not $body.access_token) { exit 0 }

$body | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $cred -Encoding UTF8

$existing = ""
if (Test-Path -LiteralPath $upmconfig) { $existing = Get-Content -LiteralPath $upmconfig -Raw }
$pattern = "(?ms)^\[npmAuth\.""" + [regex]::Escape($gateway) + "/?""\].*?(?=^\[|\z)"
$cleaned = ([regex]::Replace($existing, $pattern, "")).TrimEnd()
$block = "[npmAuth.""$gateway""]`ntoken = ""$($body.access_token)""`nalwaysAuth = true`n"
if ($cleaned) { $out = $cleaned + "`n`n" + $block } else { $out = $block }
[IO.File]::WriteAllText($upmconfig, $out)
HELPER_PS1

  sed "s|__EZG_GATEWAY__|${GATEWAY_URL}|g" "$EZG_CRED_DIR/refresh-session.sh.tmp" >"$EZG_CRED_DIR/refresh-session.sh"
  sed "s|__EZG_GATEWAY__|${GATEWAY_URL}|g" "$EZG_CRED_DIR/refresh-session.ps1.tmp" >"$EZG_CRED_DIR/refresh-session.ps1"
  rm -f "$EZG_CRED_DIR/refresh-session.sh.tmp" "$EZG_CRED_DIR/refresh-session.ps1.tmp"
...
rem Refresh the EZG session before Unity starts -- see the macOS launcher for why this cannot wait
rem until the Editor is up. Failures are ignored on purpose.
if exist "%USERPROFILE%\.ezg\refresh-session.ps1" (
  powershell -NoProfile -ExecutionPolicy Bypass -File "%USERPROFILE%\.ezg\refresh-session.ps1" >nul 2>&1
)
...
# Refresh the EZG session before Unity starts. The Package Manager reads the registry token from
# ~/.upmconfig.toml at process start and cannot sign in on its own, so a stale token here means every
# com.ezg.* package fails to resolve. Best-effort: offline or logged out, we still open the Editor.
if [ -x "$HOME/.ezg/refresh-session.sh" ]; then
    "$HOME/.ezg/refresh-session.sh" >/dev/null 2>&1 || true
fi
