...
**Workflow-backed routing (hybrid):** khi intent là scaffold thuần khớp một `/new-*` command (`/new-feature`, `/new-ui`), `/planning-task` (STEP 0a) BỎ QUA Plan subagent (tiết kiệm ~15–25K token) và ghi thin-spec `_TEMPLATE_WF.md` với field `**Backed by workflow:**` + `**Workflow args:**`. Khi thực thi, `/run-backlog` (STEP 5.0) nạp đúng command đó (và skill mà nó delegate, ví dụ `/new-ui` → `create-ui`) thay vì implement free-form. Task lai (scaffold + logic custom) dùng template M/L bình thường nhưng kèm `**Backed by workflow:**`, Plan subagent chỉ plan phần delta. WF KHÔNG phải tier — tier thật vẫn nằm trong filename/bullet để review-gating của run-backlog không đổi. **Loại trừ:** `/package-module` (đóng UPM package, push thẳng monorepo `ezg-packages` — out-of-band, không qua agent/dev; chạy tay qua skill cùng tên; ĐỪNG nhầm với `/new-package`, vốn là scaffold pack IAP trong game và CÓ đi qua routing này); `/publish-feature` + `/publish-unity-package` (publish lên Feature Hub — cũng out-of-band, ghi thẳng R2 + monorepo, chạy tay); `/auto-build-setup` (cài/upgrade module CI `ezg-autobuild` — đụng nhánh build + biến CI/CD trên GitLab, out-of-band, chạy tay qua skill cùng tên); `/new-class` (task S, Plan subagent vốn đã skip cho S + không có checklist → cứ làm task S thường); không có `/new-skill`/`/new-enemy-skill` trong project này.
...
| Dev làm việc song song | **không nên** (agent `git add -A` cho review → file dở lọt vào diff review/compile; commit chỉ lấy file của task, nhưng file hai bên cùng sửa thì commit nguyên file) | được |
