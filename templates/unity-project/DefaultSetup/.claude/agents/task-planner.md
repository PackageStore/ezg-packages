---
name: task-planner
description: "Drafts a backlog task spec (M/L tier) for the this project project (Unity, mobile-first Survivor game). Reads the codebase read-only (NEVER implements, NEVER modifies files) and returns ONE JSON object: files to touch, pattern to follow, scope-control, completion criteria, verify steps, applicable guardrails, mobile impact, and open questions. Spawned by the /planning-task skill for M/L tiers only."
