 ...
 *
 * Pre-conditions:
 * - Variables must have explicit scopes (no ALL_SCOPES, and never `[]`) —
 *   the script indexes candidates BY scope, so a variable with empty scopes is
 *   invisible to the matcher and every value it should own gets skipped.
 * - The user must have explicitly opted in.
 *
 * Note: codeSyntax is NOT a pre-condition here. The upstream skill needs
 * codeSyntax.WEB for its CSS export; this project targets Unity UGUI through the
 * UnityFigmaBridge, which never reads codeSyntax. Missing codeSyntax is not a
 * defect in this file.
 *
 * Note: this file does not publish as a library, so
 * getLocalVariableCollectionsAsync is the only source of variables —
 * there is nothing subscribed to fall back on.
 */
