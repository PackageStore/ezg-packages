...
3. Run it in the background with `--auto-model-by-tier` so each task window uses the tier-mapped model/effort (quality-first map: **mọi tier → opus**, effort scaling XS=medium → L=xhigh):
