...
1. **Đọc đầu vào:**
   ...
   - **Ground vào repo (bắt buộc, trước khi vẽ class):** đọc danh sách skill trong `.claude/skills/` (tên + description) và 2 rule `.claude/rules/core-system.md`, `.claude/rules/data-persistence.md`. Architecture PHẢI slot vào pattern có sẵn — `GameFeatureBaseController`/`FeatureBaseController`, `UIManager`, `PlayerDataManager.[Module]` + `SetupDefaultData()`, `TigerForge` + `EventName`, `TimeManager`, CSV config, `RewardManager` — KHÔNG phát minh hệ thống song song với thứ đã tồn tại (manager time riêng, event bus riêng, save layer riêng...). Class nào kế thừa/gọi hệ thống có sẵn phải ghi rõ tên hệ thống đó trong Section 4.
...
2. **Sinh cấu trúc Technical Spec:**
   ...
   - **EPIC — Section 6b bắt buộc khi có backend write:** thêm mục `6b. API Contract & Server Authority` — từng endpoint (qua Cloudflare Worker, KHÔNG direct Supabase write), request/response payload, ai là authority cho từng giá trị (server-authoritative vs client-reported), và anti-cheat model cho giá trị value-bearing.
