...
- UI-kit v0 extract từ template prefab YAML (không cần Unity sống): `python3 .claude/scripts/ui-kit-sync.py` — chạy lại khi `Assets/_Project/Visual/ArtAsset/Shared/Resources/Prefabs/Templates/` đổi (đường dẫn thật lấy từ `uiTemplatesRoot` trong `.claude/project-profile.json`); v1 (PNG chụp thật qua MCP) thay CSS background mà không đổi class/JSON shape.
